...
## 곡 선별: 빈도 가중 + 소스별 슬롯 (2026-07-15, 사용자 지정) #decision
- **발견**: v9 설계의 "중복빈도 정렬"이 백엔드 분리(v28) 재작성 때 유실 — v31은 수집된 제목 전체에서 무작위 샘플링만 하고 있었음(문서와 구현 불일치, 사용자가 발견).
- **사용자 방향**: ① "그 무드에서 몇 번 잡혔나"는 중요한 단서라 복원 ② 플레이리스트(6소스)와 DJ세션(3소스)은 곡 스타일이 달라 하나의 빈도 풀로 겨루면 세션이 항상 밀림 → **소스별 슬롯으로 몇 곡씩 섞기** ③ 같은 플리 반복→새로움 사망 문제는 유저별 수집/선별 블렌딩(기존 TODO의 explore/exploit)에서 다룸.
- **구현**: yt-search **v15** — 응답에 `split{playlist, session}` 추가(기존 `results` 합본은 유지 → 배포된 v31 웹 무영향). 웹(v32 대기분) — pickByFreq(cleanTitle 정규화 키로 빈도 집계, 점수=빈도+랜덤 가중 샘플링, 완전 빈도순은 새로움 죽여서 배제), 세션 슬롯 30%(세션 곡 있을 때만, 부족분은 플리로), 최종 정렬에 빈도 보너스(2회+0.4/3회+0.8/최대+1.2, 선호점수와 동급 이하로 캡). 구백엔드 폴백 있음.
- **검증(실데이터)**: 후보 40개 중 세션 12(정확히 30%), 중복 등장곡(2회) 2곡 모두 채택, 폴백 경로 정상. 부수 효과 — 후보 단계 중복 제거로 낭비 Spotify 검색 감소.
